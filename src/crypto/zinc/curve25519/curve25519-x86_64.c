...
{
	asm volatile("movl    $38, %%edx; " /* 2*c = 38 = 2^256 */
...
static __always_inline void sub_eltfp25519_1w(u64 *const c, const u64 *const a,
					      const u64 *const b)
{
	asm volatile("mov     $38, %%eax ;"
		     "movq   (%1),  %%r8 ;"
...
static __always_inline void mul_a24_eltfp25519_1w(u64 *const c,
						  const u64 *const a)
{
...
{
	asm volatile("test %4, %4 ;"
		     "cmovnzq %5, %0 ;"
		     "cmovnzq %6, %1 ;"
		     "cmovnzq %7, %2 ;"
		     "cmovnzq %8, %3 ;"
		     : "+r"(px[0]), "+r"(px[1]), "+r"(px[2]), "+r"(px[3])
		     : "r"(bit), "rm"(py[0]), "rm"(py[1]), "rm"(py[2]),
		       "rm"(py[3])
		     : "cc");
}
			   ...
			   const u8 session_key[CURVE25519_KEY_SIZE])
{
	...
	for (i = 3; i >= 0; --i) {
		while (j >= 0) {
			...
			sqr_eltfp25519_2w_adx(
				X3Z3); /* [X3|Z3] = [(DA+CB)|(DA+CB)]^2 */
			...
			mul_eltfp25519_2w_adx(
				X2Z2, X2Z2,
				AB); /* [X2|Z2] = [B|E]*[A|a24*E+B] */
				...
				const u8 private_key[CURVE25519_KEY_SIZE])
{
	...
	for (i = 0; i < NUM_WORDS_ELTFP25519; ++i) {
		while (j < ite[i]) {
			...
			sub_eltfp25519_1w(B, A,
					  C); /* B = (Ur1+Zr1) - M*(Ur1-Zr1) */
			add_eltfp25519_1w_adx(
				A, A, C); /* A = (Ur1+Zr1) + M*(Ur1-Zr1) */
			sqr_eltfp25519_2w_adx(AB); /* A = A^2      |  B = B^2 */
			mul_eltfp25519_2w_adx(
				UZr1, ZUr2,
				AB); /* Ur1 = Zr2*A  |  Zr1 = Ur2*B */
			++j;
		}
		j = 0;
	}
	...
	for (i = 0; i < q; ++i) {
		...
		mul_eltfp25519_2w_adx(UZr1, AB,
				      CD); /*  Ur1 = A*B   Zr1 = Zr1*A */
	}
			    ...
			    const u8 session_key[CURVE25519_KEY_SIZE])
{
	...
	for (i = 3; i >= 0; --i) {
		while (j >= 0) {
			...
			sub_eltfp25519_1w(D, X3, Z3); /* D = (X3-Z3) */
			mul_eltfp25519_2w_bmi2(DACB, AB,
					       DC); /* [DA|CB] = [A|B]*[D|C] */
				 ...
				 const u8 private_key[CURVE25519_KEY_SIZE])
{
	...
	for (i = 0; i < q; ++i) {
		...
		mul_eltfp25519_2w_bmi2(UZr1, AB,
				       CD); /*  Ur1 = A*B   Zr1 = Zr1*A */
	}
