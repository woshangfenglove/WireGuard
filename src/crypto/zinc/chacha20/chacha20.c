...
#define QUARTER_ROUND(x, a, b, c, d)                                  \
	(x[a] += x[b], x[d] = rol32((x[d] ^ x[a]), 16), x[c] += x[d], \
	 x[b] = rol32((x[b] ^ x[c]), 12), x[a] += x[b],               \
	 x[d] = rol32((x[d] ^ x[a]), 8), x[c] += x[d],                \
	 x[b] = rol32((x[b] ^ x[c]), 7))
...
#define DOUBLE_ROUND(x)                                        \
	(/* Column Round */                                    \
	 ...
	 QUARTER_ROUND(x, C(0, 3), C(1, 3), C(2, 3),           \
		       C(3, 3)), /* Diagonal Round */          \
...
#define TWENTY_ROUNDS(x)                                                     \
	(DOUBLE_ROUND(x), DOUBLE_ROUND(x), DOUBLE_ROUND(x), DOUBLE_ROUND(x), \
	 DOUBLE_ROUND(x), DOUBLE_ROUND(x), DOUBLE_ROUND(x), DOUBLE_ROUND(x), \
	 DOUBLE_ROUND(x), DOUBLE_ROUND(x))
...
{
	u32 x[] = {
		CHACHA20_CONSTANT_EXPA,	CHACHA20_CONSTANT_ND_3,
		CHACHA20_CONSTANT_2_BY,	CHACHA20_CONSTANT_TE_K,
		get_unaligned_le32(key + 0),   get_unaligned_le32(key + 4),
		get_unaligned_le32(key + 8),   get_unaligned_le32(key + 12),
		get_unaligned_le32(key + 16),  get_unaligned_le32(key + 20),
		get_unaligned_le32(key + 24),  get_unaligned_le32(key + 28),
		get_unaligned_le32(nonce + 0), get_unaligned_le32(nonce + 4),
		get_unaligned_le32(nonce + 8), get_unaligned_le32(nonce + 12)
	};
