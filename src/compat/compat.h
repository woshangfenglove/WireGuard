...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 3, 0) && !defined(DEBUG) && \
...
#define ipv6_dst_lookup(a, b, c, d) ipv6_dst_lookup(b, c, d)
     ...
     (LINUX_VERSION_CODE < KERNEL_VERSION(4, 1, 17) &&    \
...
{
	struct rng_is_initialized_callback *rdy =
		container_of(cb, struct rng_is_initialized_callback, cb);
					   ...
					   __be32 local, int scope)
{
	...
	for_each_netdev_rcu (net, dev) {
		...
		if (in_dev) {
			addr = our_confirm_addr_indev(in_dev, dst, local,
						      scope);
			if (addr)
				break;
		}
...
genl_family_attrbuf(const struct genl_family *family)
{
...
#define genl_register_family(a) \
	genl_register_family_with_ops(a, genl_ops, ARRAY_SIZE(genl_ops))
...
#define COMPAT_CANNOT_USE_GENL_NOPS
	...
	(LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, 16) &&  \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0)) || \
	(LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 65) &&   \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(4, 5, 0)) ||  \
	(LINUX_VERSION_CODE < KERNEL_VERSION(4, 4, 101) &&  \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0)) || \
...
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 8) &&       \
     LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, 0)) ||     \
	(LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 25) &&  \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0)) || \
	LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 87)
#define wg_get_device_dump(a, b)                                        \
	wg_get_device_dump_real(a, b);                                  \
...
#define timer_setup(a, b, c) \
	setup_timer(a, ((void (*)(unsigned long))b), ((unsigned long)a))
...
#include <net/genetlink.h>
#define genl_dump_check_consistent(a, b) \
	genl_dump_check_consistent(a, b, &genl_family)
#endif
				  ...
				  unsigned int size)
{
	if (IS_ENABLED(CONFIG_HAVE_EFFICIENT_UNALIGNED_ACCESS) &&
	    __builtin_constant_p(size) && (size % sizeof(unsigned long)) == 0) {
...
#define wg_expired_retransmit_handshake(a) \
	wg_expired_retransmit_handshake(unsigned long timer)
#define wg_expired_send_keepalive(a) \
	wg_expired_send_keepalive(unsigned long timer)
#define wg_expired_new_handshake(a) \
	wg_expired_new_handshake(unsigned long timer)
#define wg_expired_zero_key_material(a) \
	wg_expired_zero_key_material(unsigned long timer)
