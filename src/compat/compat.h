...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 5, \
					0) && \
	LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0)
#define ISOPENSUSE42
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, \
					0) && \
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 3, \
					0) && !defined(DEBUG) && \
	defined(net_dbg_ratelimited)
#undef net_dbg_ratelimited
#define net_dbg_ratelimited(fmt, \
			    ...) do { if (0) no_printk(KERN_DEBUG pr_fmt( \
							       fmt), \
						       ## __VA_ARGS__); \
} while (0)
#endif
...
#define ipv6_dst_lookup(a, b, c, d) ipv6_dst_lookup(b, c, d)
	...
	(LINUX_VERSION_CODE < KERNEL_VERSION(4, 3, \
					     5) && \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(4, 2, 0)) || \
	(LINUX_VERSION_CODE < KERNEL_VERSION(4, 1, \
					     17) && \
			      LINUX_VERSION_CODE > KERNEL_VERSION(3, 19, 0)) || \
	(LINUX_VERSION_CODE < KERNEL_VERSION(3, 18, \
					     27) && \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(3, 17, 0)) || \
	(LINUX_VERSION_CODE < KERNEL_VERSION(3, 16, \
					     8) && \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0)) || \
	(LINUX_VERSION_CODE < KERNEL_VERSION(3, 14, \
					     40) && \
	 LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0)) || \
	(LINUX_VERSION_CODE < KERNEL_VERSION(3, 12, \
					     54))) && !defined(ISUBUNTU1404)
#include <linux/if.h>
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 8, \
					0) && IS_ENABLED(CONFIG_IPV6) && \
	!defined(ISOPENSUSE42) && !defined(ISRHEL7)
...
static inline void *our_pskb_put(struct sk_buff *skb, struct sk_buff *tail,
				 int len)
{
	if (tail != skb) {
		skb->data_len += len;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, \
					0) && \
	LINUX_VERSION_CODE >= KERNEL_VERSION(4, 2, 0) && !defined(ISOPENSUSE15)
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, \
					0) && \
	LINUX_VERSION_CODE >= KERNEL_VERSION(4, 2, 0)
#include <linux/random.h>
...
{
	struct rng_is_initialized_callback *rdy = container_of(cb,
							       struct rng_is_initialized_callback,
							       cb);
...
static inline bool rng_is_initialized(void)
{
	...
	if (atomic_cmpxchg(&rng_state, 0, 1) == 0) {
		int ret;
		struct rng_is_initialized_callback *rdy = kmalloc(sizeof(*rdy),
								  GFP_ATOMIC);
		if (!rdy) {
...
static inline __be32 our_confirm_addr_indev(struct in_device *in_dev,
...
static inline __be32 our_inet_confirm_addr(struct net *net,
					   ...
					   __be32 local, int scope)
{
	...
	for_each_netdev_rcu(net, dev) {
		...
		if (in_dev) {
			addr =
				our_confirm_addr_indev(in_dev, dst, local,
						       scope);
			if (addr)
				break;
		}
...
#if ((LINUX_VERSION_CODE < KERNEL_VERSION(3, 15, \
					  0) && \
	LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, \
					     0)) \
	|| LINUX_VERSION_CODE < KERNEL_VERSION(3, 12, \
					       41)) && !defined(ISUBUNTU1404)
#include <linux/vmalloc.h>
#include <linux/mm.h>
...
#define genl_register_family(a) genl_register_family_with_ops(a, genl_ops, ARRAY_SIZE( \
								      genl_ops))
...
#define COMPAT_CANNOT_USE_GENL_NOPS
					 ...
					 2) && \
	LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, \
					     0)) \
	|| (LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, \
						16) \
	    && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, \
						    0)) \
	|| (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, \
						65) \
	    && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 5, \
						    0)) \
	|| (LINUX_VERSION_CODE < KERNEL_VERSION(4, 4, \
						101) \
	    && LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, \
						    0)) \
	|| LINUX_VERSION_CODE < KERNEL_VERSION(3, 18, 84)
...
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, \
					 8) && \
	LINUX_VERSION_CODE >= KERNEL_VERSION(4, 15, \
					     0)) \
	|| (LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, \
						25) \
	    && LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, \
						    0)) \
	|| LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 87)
...
#include <asm/xsave.h>
#include <asm/xcr.h>
static inline int cpu_has_xfeatures(u64 xfeatures_needed,
...
#define netdev_notifier_info net_device *) data); \
	__attribute((unused)) char _____dummy = ((struct _____dummy_container
#endif

#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0)
#define timer_setup(a, b, c) setup_timer(a, ((void (*)(unsigned long)) b), \
					 ((unsigned long)a))
#define from_timer(var, callback_timer, timer_fieldname) container_of( \
		callback_timer, typeof(*var), timer_fieldname)
#endif
...
#include <net/genetlink.h>
#define genl_dump_check_consistent(a, b) genl_dump_check_consistent(a, b, \
								    &genl_family)
#endif

#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, \
					0) && !defined(ISRHEL7) && \
	!defined(ISOPENSUSE15)
...
#define wg_expired_retransmit_handshake(a) wg_expired_retransmit_handshake( \
		unsigned long timer)
#define wg_expired_send_keepalive(a) wg_expired_send_keepalive( \
		unsigned long timer)
#define wg_expired_new_handshake(a) wg_expired_new_handshake(unsigned long timer)
#define wg_expired_zero_key_material(a) wg_expired_zero_key_material( \
		unsigned long timer)
#define wg_expired_send_persistent_keepalive(a) \
	wg_expired_send_persistent_keepalive(unsigned long timer)
#undef timer_setup
#define timer_setup(a, b, c) setup_timer(a, ((void (*)(unsigned long))b), \
					 ((unsigned long)a))
#undef from_timer
#define from_timer(var, callback_timer, \
		   timer_fieldname) container_of( \
