...
#define WG_CONFIG_SEARCH_PATHS \
	"/data/misc/wireguard /data/data/" WG_PACKAGE_NAME "/files"
#endif
...
#define DEFINE_CMD(name) \
	_cleanup_(free_command_buffer) struct command_buffer name = { 0 };
...
static void del_if(const char *iface)
{
	...
	char *netid = NULL;
	_cleanup_free_ char *regex =
		concat("0xc([0-9a-f]+)/0xcffff lookup ", iface, NULL);

	xregcomp(&reg, regex, REG_EXTENDED);

	cmd("iptables -D OUTPUT -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"wireguard rule %s\"",
	    iface);
	cmd("ip6tables -D OUTPUT -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"wireguard rule %s\"",
	    iface);
...
static void up_if(unsigned int *netid, const char *iface)
{
	...
	cmd("iptables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"wireguard rule %s\"",
	    iface);
	cmd("ip6tables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"wireguard rule %s\"",
	    iface);
...
static void maybe_block_ipv6(const char *iface)
{
	...
	if (listenport > UINT16_MAX || !listenport)
		goto set_back_down;
	cmd("ip6tables -I INPUT 1 -p udp --dport %lu -j DROP -m comment --comment \"wireguard rule %s\"",
	    listenport, iface);
set_back_down:
...
static void maybe_unblock_ipv6(const char *iface)
{
	...
	_cleanup_regfree_ regex_t reg = { 0 };
	_cleanup_free_ char *regex =
		concat("^-A (.* --comment \"wireguard rule ", iface,
		       "\"[^\n]*)\n*$", NULL);
	DEFINE_CMD(c);
...
static void set_config(const char *iface, const char *config)
{
	...
	_cleanup_free_ char *cmd =
		concat("wg setconf ", iface, " /proc/self/fd/0", NULL);
	int ret;
		...
		if (in_interface_section) {
			if (!strncasecmp(clean, "Address=", 8) && j > 8) {
				...
				*dnses =
				...
				continue;
			} else if (!strncasecmp(clean,
						"ExcludedApplications=", 21) &&
				   j > 4) {
				*excluded_applications =
					concat_and_free(*excluded_applications,
							",", clean + 21);
				continue;
