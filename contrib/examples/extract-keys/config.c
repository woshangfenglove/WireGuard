...
const struct def defs[] = {
	...
	{ "DEVICE_NAME_OFFSET",
	  -ALIGN(sizeof(struct net_device), NETDEV_ALIGN) + offsetof(
		  struct net_device, name) },
	{ "IFNAMSIZ", IFNAMSIZ },
	{ "DEVICE_PEERS_OFFSET", offsetof(struct wg_device, peer_list) },
	{ "PEERS_PEER_OFFSET", -offsetof(struct wg_peer, peer_list) },
	{ "PEER_CURRENTKEY_OFFSET", offsetof(struct wg_peer,
					     keypairs.current_keypair) },
	{ "PEER_PREVIOUSKEY_OFFSET", offsetof(struct wg_peer,
					      keypairs.previous_keypair) },
	{ "PEER_NEXTKEY_OFFSET",
	  offsetof(struct wg_peer, keypairs.next_keypair) },
	...
	{ "KEY_RECEIVING_OFFSET",
	  offsetof(struct noise_keypair, receiving.key) },
	{ NULL, 0 }
};
#else
#include <stdio.h>
