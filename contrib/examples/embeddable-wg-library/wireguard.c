				 ...
				 size_t len)
{
	...
	return sendto(nl->fd, buf, len, 0, (struct sockaddr *)&snl,
		      sizeof(snl));
}
...
again:
	...
	for (peer = peer ? peer : dev->first_peer; peer;
		...
		peer_nest = mnl_attr_nest_start_check(
			nlh, MNL_SOCKET_BUFFER_SIZE, 0);
		if (!peer_nest)
			goto toobig_peers;
		if (!mnl_attr_put_check(
			    nlh, MNL_SOCKET_BUFFER_SIZE, WGPEER_A_PUBLIC_KEY,
