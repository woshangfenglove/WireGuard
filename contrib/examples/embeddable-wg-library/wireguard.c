...
#define MNL_SOCKET_BUFFER_SIZE (sysconf(_SC_PAGESIZE) < \
				8192L ? sysconf(_SC_PAGESIZE) : 8192L)
...
#define mnl_attr_for_each_payload(payload, payload_size) \
	for ((attr) = (payload); \
	     mnl_attr_ok((attr), \
			 (char *)(payload) + payload_size - (char *)(attr)); \
	     (attr) = mnl_attr_next(attr))
...
again:
	...
	for (peer = peer ? peer : dev->first_peer; peer;
		...
		if (flags)
			if (!mnl_attr_put_u32_check(nlh, MNL_SOCKET_BUFFER_SIZE,
...
int wg_key_from_base64(wg_key key, const wg_key_b64_string base64)
{
	...
	val = decode_base64((const char[]){ base64[i * 4 + 0],
					    base64[i * 4 + 1],
					    base64[i * 4 + 2], 'A' });
